...
RUN_TAGGER_CMD = "java -XX:ParallelGCThreads=2 -Xmx1024m -jar ark-tweet-nlp-0.3.2.jar"
...
def _call_runtagger(tweets, run_tagger_cmd=RUN_TAGGER_CMD):
    ...
    # write file to an actual tempfile for java to read
    with open('./temp/cmu_tagger_temp', 'wb') as outf:
        outf.write(message)
    ...
    args.append('conll')
    args.append('./temp/cmu_tagger_temp')
    ...
    #print(result)
    #print(type(result))
    pos_result = result.decode()
    #print(pos_result)
    pos_result = pos_result.strip()
    #print(pos_result)
    pos_results = pos_result.split('\n\r')
    #print(pos_results)
    pos_results = [tweet.strip().split('\n') for tweet in pos_results]
    #print(pos_results)
    pos_results_list = [[tuple(word.split('\t')) for word in tweet] for tweet in pos_results]
    #print(pos_results_list)
